...
//        System.out.println(test.deleteAndEarn3(nums)); // error
        ...
        System.out.println(test.deleteAndEarn5(nums));
...
//        System.out.println("数组打印："+Arrays.toString(dp));
    ...
    public int deleteAndEarn5(int[] nums) {
        ...
        int one = use[0] * keyNum.get(use[0]);
        int two =  Math.abs(use[0] - use[1]) == 1 ?
                Math.max(use[0] * keyNum.get(use[0]),use[1] * keyNum.get(use[1])) :
                use[0] * keyNum.get(use[0]) + use[1] * keyNum.get(use[1]);
        int temp = 0;
        for (int i = 2; i < newLen; i++){

            if (two != one && Math.abs(use[i] - use[i - 1]) == 1){
                temp = Math.max(two,one + use[i] * keyNum.get(use[i]));
            }else {
                temp = two + use[i] * keyNum.get(use[i]);
            }
            one = two;
            two = temp;
        }
